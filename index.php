...
$f3->route('POST /login',
	function($f3) {
		...
		$auth = new \Auth(new \DB\SQL\Mapper($db, 'accounts'), 
...
$f3->route('GET /main',
	function($f3) {
		if (!$f3->exists('SESSION.email')) $f3->reroute('/');
		...
		$account = new DB\SQL\Mapper($db, 'accounts');
		$account->load(array('email=?', $f3->get('SESSION.email')));
...
$f3->route('GET /profile/@id',
	function($f3) {
		...
		$f3->set('education_levels', ['PRIMARY', 'HIGH_SCHOOL', 'SOME_COLLEGE', 'COLLEGE', 'GRADUATE', 'DECLINE']);
...
$f3->route('GET /club/@id',
	function($f3) {
		...
		$f3->set('isClubManager', $club->creator_account_id == $account->account_id);
		$f3->set('canRegisterTeam', $club->creator_account_id == $account->account_id);
...
$f3->route('GET /team/@id',
	function($f3) {
		...
		$players = $db->exec(
			'SELECT pf.*, t2.team_id, t2.name as team_name
			 FROM profiles pf
			 ...
			 LEFT JOIN teams t2 
			 	ON p.team_id = t2.team_id 
			 	AND t.tournament_id = t2.tournament_id
		...
		$roster = $db->exec(
			'SELECT pf.* 
			 FROM profiles pf, players p 
			 WHERE pf.account_id = p.account_id
			 AND p.team_id = ?', $team->team_id);
		$f3->set('roster', $roster);
		
		$nonroster = $db->exec(
			'SELECT pf.* 
			 FROM profiles pf, teams t, club_members cm 
			 WHERE pf.account_id = cm.account_id 
			 AND cm.account_id NOT IN (
			 	SELECT account_id
			 	FROM players
			 	WHERE team_id = ?)
			 AND cm.status = "ACTIVE"
			 AND cm.club_id = t.club_id
			 AND t.team_id = ?', array(1=>$team->team_id, 2=>$team->team_id));
		$f3->set('nonroster', $nonroster);
		**/
